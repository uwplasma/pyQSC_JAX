...
def test_target_iota_etabar_round_trip_and_sign_branches():
    ...
    negative = qsc.solve(axis=AXIS, etabar=-1.0, iota=forward.iota, solve_for="etabar", nphi=31)
    positive = qsc.solve(axis=AXIS, etabar=1.0, iota=forward.iota, solve_for="etabar", nphi=31)
...
def test_target_iota_exposes_distinct_local_etabar_branches():
    ...
    outer = qsc.solve(axis=AXIS, etabar=-1.0, iota=target, solve_for="etabar", nphi=31)
    inner = qsc.solve(axis=AXIS, etabar=-0.8, iota=target, solve_for="etabar", nphi=31)
...
def test_target_iota_I2_round_trip(actual_I2):
    ...
    inverse = qsc.solve(axis=AXIS, etabar=-0.9, I2=0.0, iota=forward.iota, solve_for="I2", nphi=31)
...
def test_inverse_mode_propagates_to_second_order():
    ...
    inverse = qsc.solve(
        axis=AXIS, etabar=-1.0, I2=0.2, iota=forward.iota, solve_for="etabar", nphi=31, order="r2"
    )
...
def test_inverse_solve_is_jittable_and_implicitly_differentiable():
    def etabar_for_iota(target_iota):
        return qsc.solve(
            axis=AXIS, etabar=-1.0, iota=target_iota, solve_for="etabar", nphi=31
        ).inputs.etabar

    target = qsc.solve(axis=AXIS, etabar=-0.9, nphi=31).iota
    solution = qsc.solve(axis=AXIS, etabar=-1.0, iota=target, solve_for="etabar", nphi=31)
...
def test_inverse_input_guards_and_forward_diagnostic_absence():
    ...
    with pytest.raises(ValueError, match="scalar"):
        qsc.solve(axis=AXIS, etabar=-1.0, iota=jnp.ones(2), solve_for="etabar")
    with pytest.raises(ValueError, match="fold_tolerance"):
        qsc.solve(axis=AXIS, etabar=-1.0, iota=0.4, solve_for="etabar", fold_tolerance=-1.0)
