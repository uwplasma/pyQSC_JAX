...
def test_stellarator_symmetric_indices_and_low_discrepancy_box():
    ...
    np.testing.assert_allclose(samples, [[0.5, 1 / 3, 0.2], [0.25, 2 / 3, 0.4], [0.75, 1 / 9, 0.6]])
...
def test_fourier_continuation_copies_retained_modes_and_runs_stages():
    ...
    continuation = qsc.continue_axis_search(
        ...
        options=small_options(
            coarse_samples=1, maximum_iterations=1, verification_multipliers=(1,)
        ),
    )
...
def test_search_certifies_only_the_verified_nonnegative_zero():
    ...
    assert not _candidate_is_distinct(
        result.best.variables, [result.best], jnp.asarray([0.8]), jnp.asarray([1.2]), 1.0e-3
    )
...
def test_search_failure_and_ill_conditioning_are_distinct_statuses(monkeypatch):
    coarse_failure = qsc.search_axis(
        ...
        options=small_options(coarse_samples=1, verification_multipliers=(1,)),
    )
    ...
    with monkeypatch.context() as context:
        context.setattr(axis_optimization, "_levenberg_marquardt", forced_nonfinite_local_solve)
...
def test_search_reports_no_feasible_candidate_for_failed_hard_profile():
    ...
    result = qsc.search_axis(
        circular_problem(criteria=impossible, selector="maximum_criteria_margin"),
...
def test_every_canonical_selector_has_documented_orientation():
    ...
    np.testing.assert_allclose(
        _selector_value(replace(base, selector="maximum_singular_radius"), solution, report),
        solution.r_singularity,
    )
    np.testing.assert_allclose(
        _selector_value(replace(base, selector="maximum_minimum_L_grad_B"), solution, report),
        np.min(solution.L_grad_B),
    )
    np.testing.assert_allclose(
        _selector_value(replace(base, selector="maximum_minimum_L_grad_grad_B"), solution, report),
        np.min(solution.L_grad_grad_B),
    )
    np.testing.assert_allclose(
        _selector_value(replace(base, selector="minimum_maximum_elongation"), solution, report),
        -np.max(solution.elongation),
    )
    assert (
        _selector_value(replace(base, selector="minimum_axis_sobolev_norm"), solution, report) < 0
    )
    assert (
        _selector_value(
            replace(
                base, selector="target_axis_length", target_axis_length=float(solution.axis_length)
            ),
            solution,
            report,
        )
    ...
    assert np.isfinite(
        _selector_value(
            replace(base, criteria=criteria, selector="maximum_criteria_margin"), solution, report
        )
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (circular_problem(target_iota=0.1), small_options(), "target_iota"),
        (circular_problem(solve_for="etabar"), small_options(), "target_iota"),
        (circular_problem(solve_for="bad"), small_options(), "solve_for"),
        (circular_problem(selector="target_axis_length"), small_options(), "target_axis_length"),
        (circular_problem(selector="maximum_criteria_margin"), small_options(), "criteria"),
        (circular_problem(selector="bad"), small_options(), "selector"),
        (circular_problem(), small_options(coarse_samples=0), "positive integers"),
        (circular_problem(), small_options(initial_damping=0.0), "must be positive"),
        (circular_problem(), small_options(verification_multipliers=()), "nonempty"),
    ],
)
...
def test_valid_inverse_search_policy_reaches_post_branch_validation():
    _validate_problem(circular_problem(solve_for="I2", target_iota=0.1), small_options())


def test_explicit_seed_guard():
    with pytest.raises(ValueError, match="explicit seed"):
        qsc.search_axis(circular_problem(), options=small_options(), seeds=(jnp.asarray([1.3]),))
