...
def circular_solution(*, I2=0.1, p2=0.0, nphi=31):
    return qsc.Qsc(rc=[1.0], zs=[0.0], nfp=1, etabar=1.0, I2=I2, p2=p2, nphi=nphi, order="r2")
...
def test_regularized_integral_resolution_convergence_for_nonplanar_axis():
    coarse = qsc.Qsc(rc=[1.0, 0.045], zs=[0.0, -0.045], nfp=3, etabar=-0.9, nphi=31, order="r1")
    fine = qsc.Qsc(rc=[1.0, 0.045], zs=[0.0, -0.045], nfp=3, etabar=-0.9, nphi=61, order="r1")
...
def test_documented_plasma_dominant_case_exceeds_thirty_percent():
    ...
    np.testing.assert_allclose(
        plasma.current_source.enclosed_toroidal_current, 840000.0, rtol=2.0e-13
    )
