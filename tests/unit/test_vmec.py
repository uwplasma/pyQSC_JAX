...
def qa_solution(*, nphi: int = 61):
    return qsc.Qsc(rc=[1.0, 0.045], zs=[0.0, -0.045], nfp=3, etabar=-0.9, nphi=nphi, order="r2")
...
def test_vectorized_surface_matches_independent_legacy_root_solve():
    ...
    new_R, new_Z, new_phi0, residual = qsc.uniform_cylindrical_surface(
        field.solution, radius, ntheta=8
    )
...
def test_asymmetric_axis_emits_all_four_vmec_coefficient_families(tmp_path):
    ...
    export = qsc.to_vmec(solution, tmp_path / "input.asymmetric", r=0.01, ntheta=16, mpol=4, ntor=4)
...
def test_first_and_third_order_surfaces_and_ntor_cap(tmp_path):
    first_order = qsc.Qsc(
        rc=[1.0, 0.045], zs=[0.0, -0.045], nfp=3, etabar=-0.9, nphi=15, order="r1"
    )
...
def test_to_vmec_resolution_and_scalar_guards(tmp_path, kwargs, message):
    options = {"r": 0.01, "ntheta": 8, "mpol": 3, "ntor": 2}
    options.update(kwargs)
    with pytest.raises(ValueError, match=message):
        qsc.to_vmec(qa_solution(nphi=15), tmp_path / "input.invalid", **options)
