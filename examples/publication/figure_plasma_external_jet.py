...
surface_axis.set_title(
    ...
    rf"$\tau_\mathrm{{rms}}={float((solution.torsion**2).mean() ** 0.5):.3f}$ m$^{{-1}}$"
)

fraction_axis = figure.add_subplot(2, 2, 2)
fraction_percent = 100.0 * np.asarray(plasma_fraction)
fraction_axis.plot(angle, fraction_percent, color="tab:red", linewidth=2.3)
fraction_axis.fill_between(angle, 0.0, fraction_percent, alpha=0.18, color="tab:red")
fraction_axis.set_xlabel("Boozer angle / field period")
fraction_axis.set_ylabel(r"$100|B_\mathrm{plasma}|/|B_\mathrm{total}|$ [%]")
fraction_axis.set_title("pressure-driven plasma contribution")
...
metadata = {
    ...
    "torsion_minimum": float(solution.torsion.min()),
    "torsion_maximum": float(solution.torsion.max()),
