...
export = qsc.to_vmec(solution, VMEC_INPUT, r=RADIUS, ntheta=NTHETA, mpol=MPOL, ntor=NTOR)
...
for index in phi_indices:
    axes[0].plot(np.asarray(boundary.R[:, index]), np.asarray(boundary.Z[:, index]), linewidth=1.3)
