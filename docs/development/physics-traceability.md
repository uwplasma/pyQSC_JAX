...
| Target-transform inverse solve | implicit-function reformulation of the first-order sigma equation | `inverse.solve_target_iota` | `etabar` and `I2` round trips, sign branches, distinct local basins, fold metric, r2 propagation, JIT/JVP/finite differences | branch-local solve implemented; pseudo-arclength continuation pending |
